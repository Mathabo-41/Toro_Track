...
export default function ProjectsContent() {
  ...
  const {
    projects, clients, searchTerm, setSearchTerm, openCreateDialog, setOpenCreateDialog, newProject,
    anchorEl, selectedProject, teamMembers, filteredProjects, handleMenuOpen, handleMenuClose,
    handleDeleteProject, handleStatusChange, handleCreateProject, handleInputChange,
    updateProject
  } = useProjects();
  ...
  const handleOpenEditDialog = (projectId) => {
    ...
    if (projectToEdit) {
      setEditProject({
        ...projectToEdit,
        // Convert team array to a comma-separated string for the text field
  ...
  const handleSaveEdit = async () => {
    if (editProject) {
      // Convert the team string back to an array before sending to the service
      const teamAsArray = editProject.team.split(',').map(item => item.trim()).filter(Boolean);
      const payload = { ...editProject, team: teamAsArray };
  ...
  return (
    <Box sx={styles.container}>
      <Drawer variant="permanent" anchor="left" sx={styles.sidebar}>
         <Box sx={{ p: 1, borderBottom: '2px solid #6b705c', display: 'flex', alignItems: 'center', gap: 1 }}>
            <Link href="/dashboard" passHref>
              <IconButton sx={{ color: 'green' }} aria-label="Go to Dashboard"><DashboardIcon /></IconButton>
            </Link>
              ...
              <Table>
                  <TableHead>
                      <TableRow>
                          ...
                          <TableCell sx={styles.tableHeaderCell}>Assign Team</TableCell>
                          <TableCell sx={styles.tableHeaderCell}>Update Project</TableCell>
                      </TableRow>
                  </TableHead>
                  <TableBody>
                          ...
                          <TableRow key={project.id} hover sx={styles.tableRowHover}>
                              ...
                              <TableCell>
                                <Stack direction="row" spacing={1} flexWrap="wrap" gap={0.5}>
                                  {(project.team || []).map((role, i) => (<Chip key={i} label={role} size="small" />))}
                                </Stack>
                              </TableCell>
      ...
      {/* Edit Project Dialog */}
      <Dialog open={openEditDialog && !!editProject} onClose={() => setOpenEditDialog(false)} fullWidth maxWidth="sm" PaperProps={{ sx: styles.createDialogPaper }}>
        <DialogTitle sx={styles.createDialogTitle}>
            <Typography component="span" variant="h6">Update Project</Typography>
            <IconButton onClick={() => setOpenEditDialog(false)}><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent sx={{ ...styles.dialogContent, mt: 3, px: 3 }}>
            {editProject && (
                <Stack spacing={3}>
                    <TextField fullWidth label="Project Name" name="name" value={editProject.name} onChange={handleEditInputChange} sx={styles.dialogTextField} />
                    <FormControl fullWidth>
                        <InputLabel sx={styles.dialogInputLabel}>Client</InputLabel>
                        <Select name="client" value={editProject.client} onChange={handleEditInputChange} sx={styles.dialogSelect}>
                            {clients.map(client => (<MenuItem key={client.id} value={client.client_name}>{client.client_name}</MenuItem>))}
                        </Select>
                    </FormControl>
                    <FormControl fullWidth>
                    ...
                    <TextField fullWidth name="team" label="Assign Team" placeholder="e.g., Business Analyst, DevOps Team" value={editProject.team} onChange={handleEditInputChange} sx={styles.dialogTextField}/>
                </Stack>
            )}
        </DialogContent>
      ...
      <Dialog open={openCreateDialog} onClose={() => setOpenCreateDialog(false)} fullWidth maxWidth="sm" PaperProps={{ sx: styles.createDialogPaper }}>
          <DialogTitle sx={styles.createDialogTitle}>
          ...
          <DialogContent sx={{...styles.dialogContent, mt: 3, px: 3}}>
              <Stack spacing={3}>
                  <TextField fullWidth label="Project Name" name="name" value={newProject.name} onChange={handleInputChange} sx={styles.dialogTextField} />
                  <FormControl fullWidth>
                      <InputLabel sx={styles.dialogInputLabel}>Assign to Client</InputLabel>
                      <Select name="client" value={newProject.client} onChange={handleInputChange} sx={styles.dialogSelect}>
                          {clients.map(client => (<MenuItem key={client.id} value={client.client_name}>{client.client_name}</MenuItem>))}
                      </Select>
                  </FormControl>
                  <FormControl fullWidth>
                      <InputLabel sx={styles.dialogInputLabel}>Status</InputLabel>
                      <Select name="status" value={newProject.status} onChange={handleInputChange} sx={styles.dialogSelect}>
                          {PROJECT_STATUSES.map(status => (<MenuItem key={status} value={status}>{formatStatus(status)}</MenuItem>))}
                      </Select>
                  </FormControl>
                  <TextField fullWidth type="date" label="Due Date" name="dueDate" InputLabelProps={{ shrink: true }} value={newProject.dueDate} onChange={handleInputChange} sx={styles.dialogTextField} />
                  <TextField fullWidth name="team" label="Assign Team" placeholder="e.g., Business Analyst, DevOps Team" value={newProject.team} onChange={handleInputChange} sx={styles.dialogTextField} />
              </Stack>
          </DialogContent>
          <DialogActions sx={styles.dialogActions}>
              <Button onClick={() => setOpenCreateDialog(false)} sx={styles.dialogCancelButton}>Cancel</Button>
              <Button onClick={handleCreateProjectWithNotification} variant="contained" sx={styles.dialogCreateButton}>Create Project</Button>
          </DialogActions>
      </Dialog>

      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose} PaperProps={{ sx: styles.actionMenu }}>
          <MenuItem onClick={() => handleOpenEditDialog(selectedProject)} sx={styles.menuItem}><EditIcon sx={styles.menuItemIcon('#2196f3')} /> Editi Project</MenuItem>
